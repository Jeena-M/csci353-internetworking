 ...
 */
static
void talk_to_client(int newsockfd, int conn_number, int N)
{ 
    int bytes_received;
    string line;
    ...
    for (;;){
        ...
        for (;;) {
            ...
            if (bytes_received <= 0){
                ...
                string line_closed = "["+get_timestamp_now()+"] " + "[" + to_string(conn_number) + "]\tConnection closed with client at " + client_ip_and_port + "\n";
        ...
        if (!header_lines.empty()) {
            ...
            string line_connect =  "["+get_timestamp_now()+"] " + "[" + to_string(conn_number) + "]\tClient connected from "+ client_ip_and_port + " and requesting " +uri + "\n";
            ...
            if (size == -1){
                ...
                if (fd == -1) {
                    ...
                    while (X > 0) {
                        ...
                        numIterations += 1;
                        string line_sent = "["+get_timestamp_now()+"] " + "[" +to_string(conn_number) + "]\tSent " + to_string(numIterations) + " KB to " + client_ip_and_port + "\n";
