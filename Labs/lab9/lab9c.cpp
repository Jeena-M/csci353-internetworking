    ...
    while(true){
       header_lines.clear();
        ...
        if (!header_lines.empty()) {
            ...
            shared_ptr<Message> msg = make_shared<Message>(method, uri, version);
...
static void write_from_client(shared_ptr<Connection> conn_ptr)
{
    ...
    while(!exit){
        ...
        string path = "lab4data/" + msg->uri; // Create a file system path
        string uri = msg->uri;
                ...
                cerr << "Could not open file" << endl; 
                    ...
                    while (X > 0) {
                        ...
                        //for (int i = 0; i < bytes_actually_read; i++) {
                            //better_write(conn_ptr->socket_fd, &buf[i], 1); 
                            //usleep(250); 
                        //}

                        conn_ptr->m->lock();
                        conn_ptr->bytes_sent += 1;
                        conn_ptr->m->unlock();
    ...
    while (true) {
        ...
        if (command == "status"){
                ...
                for (const auto& conn_ptr : connection_list) {
                    if (conn_ptr->socket_fd >= 0) {  // Skip inactive connections
                        cout << "[" << conn_ptr->conn_number << "]\tClient: "
                             << get_ip_and_port_for_client(conn_ptr->orig_socket_fd, 1)
                             << ", Socket: " << conn_ptr->socket_fd
                             << ", Bytes sent: " << (conn_ptr->bytes_sent) << "\n";
                    }
                }
            }
            m.unlock();
            ...
            */
            ...
            string number = command.substr(6);
            ...
            if (hasConnection && targetConnection != nullptr && targetConnection->socket_fd >= 0){
                m.unlock();
                targetConnection->m->lock();
                shutdown(targetConnection->socket_fd, SHUT_RDWR);
                targetConnection->socket_fd = -2;
                targetConnection->m->unlock();
                m.lock();
                cout << "[" << conn << "]\tClose connection requested.\n";
            } else {
                cout << "Invalid or inactive connection number: #" << number << "\n";
            }
            m.unlock();
        } else if (command == ""){
            continue;
