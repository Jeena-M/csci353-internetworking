 ...
 */
static
void talk_to_client(int newsockfd, string rootdir)
{ 
    int bytes_received;
    string line;
    ...
    for (;;){
        ...
        for (;;) {
            ...
            if (bytes_received <= 0){
                string line_close = "["+get_timestamp_now()+"]" + " CLOSE: " + get_ip_and_port_for_server(newsockfd, 0) + "\n";
        ...
        if (!header_lines.empty()) {
            ...
            string line_request = "["+get_timestamp_now()+"]" + " REQUEST: " + get_ip_and_port_for_server(newsockfd, 0) + ", uri=" + uri + "\n";
            LogALine(line_request);

            // Log the HTTP request header 
            for (string& header_line : header_lines){
                LogALine( "\t"+header_line);
            }
            ...
            if (spaceCount != 2){
                //cout << "Here 1" << endl;
                string line_response = "["+get_timestamp_now()+"] RESPONSE: " + get_ip_and_port_for_server(newsockfd, 0) + ", status=" + "404\n";
                LogALine(line_response);
                
                LogALine("\tHTTP/1.1 404 Not Found\r\n\tServer: pa2 (jeenamah@usc.edu)\r\n\tContent-Type: text/html\r\n\tContent-Length: 63\r\n\tContent-MD5: 5b7e68429c49c66e88489a80d9780025\r\n\t\r\n");
            ...
            */
            ...
            if (size == -1){
                ...
                string line_response = "["+get_timestamp_now()+"] RESPONSE: " + get_ip_and_port_for_server(newsockfd, 0) + ", status=" + "200\n";
                LogALine(line_response);
...
void talk_to_user_and_server(int client_socket_fd, vector<string>& uris, string host, int port, vector<string>& outputfiles)
{
    ...
    for (int j = 0; j < uris.size(); j++){
        ...
        string request = "GET " + uri + " HTTP/1.1\r\nHost: " + host + ":" + to_string(port)+ "\r\nAccept: text/html, */*\r\nUser-Agent: pa2 (jeenamah@usc.edu)\r\n\r\n";
        ...
        if (bytes_sent == -1){
            cerr << "Failed to send request for "+ uri + " to server at " + host + ":" + to_string(port)+ ".\n";
            continue;
        }
        ...
        if (header_lines.empty()){ // Continue to the next URI if cannot parse response header
            cerr << "Failed to download " + uri + " from server at " + host + ":" + to_string(port) + ".\n";
            continue; 
        }
        ...
        if (version.substr(0, 5) != "HTTP/") {
            cerr << "Unrecognized response from server at " + host + ":" + to_string(port)+ ". Program terminated\n";
            shutdown(client_socket_fd, SHUT_RDWR);
            close(client_socket_fd);
            return;
        }
        ...
        if (isContentLength){ // If Content-Length was found:
            ...
            if (fd == -1) {
                cerr << "Cannot open " +  outputfile + " for writing. Download for " + uri + " is skipped.\n"; 
            } else {
                /*
                char buf[1024];
                int X = stoi(value);
                ...
                while (X > 0) {
                    ...
                    if (bytes_actually_read <= 0) { // Cannot read a complete response body - continue to next URI
                        cerr << "Failed to download " + uri + " from server at " + host + ":" + to_string(port) + ".\n";
                        break; 
                    }
                ...
                */
                ...
                char buf[1]; // Read only 1 byte at a time
                int X = stoi(value);

                int BC = 0;
                int CL = X;
                double P = 0.0;
                double avg = 0.0; // Throughput
                ...
                string progress_bar = "0 B of " + to_string(CL) + " B (0%) downloaded at 0 B/s (EWMA), time elapsed: 0 s";
                int length_of_last_line = progress_bar.length();
                struct timeval last_line_time = start_time;
                int bytes_since_last_clock = 0;
                double alpha = 0.85;
                int numTimes = 0;
                double first_sample_TP;

                cerr << progress_bar;
                cerr.flush();

                while (X > 0) {
                    int bytes_to_read = (X > 1) ? 1 : X; /* cannot read more than the size of the buffer */
                    int bytes_actually_read = read(client_socket_fd, buf, bytes_to_read);
                    ...
                    // Check elapsed time
                    bytes_since_last_clock += bytes_actually_read;
                    struct timeval now;
                    gettimeofday(&now, NULL);
                    double elapsed_time_since_last_time_stamp = timestamp_diff_in_seconds(&last_line_time, &now);
                    if (elapsed_time_since_last_time_stamp >= ((double)0.1)){
                        numTimes +=1;
                        cerr << "\r";  // Clear previous line
                        cerr.flush();

                        cerr << string(length_of_last_line, ' ');
                        cerr.flush();
                        ...
                        BC += bytes_since_last_clock;
                        P = 1.0*BC/CL * 100;
                        elapsed_time = timestamp_diff_in_seconds(&start_time, &now);

                        double sample_TP = bytes_since_last_clock/elapsed_time_since_last_time_stamp;
                        avg = (1 - alpha) * avg + alpha * sample_TP;

                        if (numTimes == 1){
                            first_sample_TP = sample_TP;
                        }

                        progress_bar = to_string(BC) + " B of " + to_string(CL) + " B ("+ to_string(P) +"%) downloaded at " + to_string(avg) + " B/s (EWMA), time elapsed: "+ to_string(elapsed_time) + " s";
                        cerr << progress_bar;
                        cerr.flush();

                        // Resets:
                        bytes_since_last_clock = 0;
                        length_of_last_line = progress_bar.length();
                        last_line_time = now;
                ...
                gettimeofday(&now, NULL);
                double elapsed_time_since_last_time_stamp = timestamp_diff_in_seconds(&last_line_time, &now);
                cerr << "\r" << string(length_of_last_line, ' ') << "\r";  // Clear previous line
                cerr.flush();
                BC += bytes_since_last_clock;
                P = 1.0*BC/CL * 100;
                elapsed_time = timestamp_diff_in_seconds(&start_time, &now);
                ...
                if (elapsed_time_since_last_time_stamp >= 0.1){
                    avg = (1 - alpha) * avg + alpha * sample_TP; // Only update if elapsed time >= 0.1, else discard
                }

                if (numTimes == 1 && (elapsed_time_since_last_time_stamp < 0.1)){
                    avg = first_sample_TP;
                }
                ...
                cerr << "\n";
                /* Close Attempt at Progress Bar */
                //cerr << numTimes << endl;
                cout << uri + " saved into " + outputfile + ".\n"; // Success message

                close(fd);
            }
        } else { // Content-Length was not found (print error and terminate client)
            cerr << "No Content-Length in response header when downloading " + uri + " from server at " + host + ":" + to_string(port) + ". Program terminated.\n";
            shutdown(client_socket_fd, SHUT_RDWR);
            close(client_socket_fd);
            return;
        }
...
int main(int argc, char *argv[])
{
    if (strcmp(argv[1], "-c") == 0){
        // User wants to run pa2 as a simple web client 
        string host = argv[2];
        ...
        if (client_socket_fd == (-1)) {
            cerr << "Failed to connect to server at " << host << ":" << argv[3] << ". Program terminated.\n";
            exit(-1);
        } else {
            string client_ip_and_port = get_ip_and_port_for_client(client_socket_fd, 1);
            cout << "pa2 client at " + client_ip_and_port + " connected to server at " + host + ":" + argv[3] +"\n";
        ...
        // User wants to run pa2 as a simple web server

        // Step 1: Parse commandline arguments: 
        ...
        // Declare variables for differet keys

        // To deal with logfile
        bool logToCout = true;
        ...
        // Find [startup] section and parse keys
        int index = 0;

        // a) Find location of [startup] in file_lines
        for (string line: file_lines){
            if (line[0] == '['){
                string curr_sec_name = line.substr(1, line.length()-3);
                if (curr_sec_name == "startup"){
                    break;
                }
            }
            index += 1;
        ...
        // b) Assign appropriate values to the variables
        int i = index + 1;
        ...
        // Step 2) Open logfile (if logToFile is true)
        ...
        // Step 3) Set up server's infinite loop correctly:
        ...
        if (listen_socket_fd != (-1)) {
            ...
            for (;;) {
                ...
                string line_connect = "["+get_timestamp_now()+"]" + " CONNECT: " + get_ip_and_port_for_server(newsockfd, 0) + "\n";
                LogALine(line_connect);
                talk_to_client(newsockfd, rootdir);
            }
